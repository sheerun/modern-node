...
function getPackageName (installPackage) {
  if (installPackage.match(/^.+\.(tgz|tar\.gz)$/)) {
    return getTemporaryDirectory()
      ...
      .catch(err => {
        // The package name could be with or without semver version, e.g. modern-node-0.2.0-alpha.1.tgz
    ...
    // git+https://github.com/mycompany/modern-node.git
    // git+ssh://github.com/mycompany/modern-node.git#v1.2.3
    ...
    console.error(
      chalk.red(
        'You are running Node %s.\n' +
          'Create Modern Node requires Node %s or higher. \n' +
...
function checkAppName (appName) {
  ...
  const dependencies = ['modern-node'].sort()
